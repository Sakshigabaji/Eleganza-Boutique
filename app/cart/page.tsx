...
export default function CartPage() {
  ...
  const totalPrice = cart.reduce(
    (sum: number, item: any) =>
      sum +
      Number(item.price?.replace(/[^0-9]/g, "") || 0) *
      (item.quantity || 1),
    0
  );
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        {cart.length === 0 ? (
          ...
          <div className="text-center bg-white/50 backdrop-blur-md p-20 rounded-3xl border border-[#C9A227]/20 max-w-2xl mx-auto shadow-xl">
            ...
            <p className="text-gray-500 mb-8 leading-relaxed">It looks like you haven't added any treasures to your collection yet.</p>
            <Link href="/" className="inline-block bg-[#311B14] text-[#C9A227] px-12 py-4 rounded-full font-bold text-xs uppercase tracking-widest hover:bg-[#C9A227] hover:text-black transition-all">
              ...
              {cart.map((item: any) => (
                ...
                >
                  ...
                  <div className="flex-1 text-center sm:text-left">
                    ...
                    <p className="text-[#C9A227] font-medium mt-1">₹{item.price.toLocaleString()}</p>
                  </div>
                  ...
                  <div className="hidden sm:block text-right min-w-[100px]">
                    ...
                    <p className="font-bold text-[#311B14]">
                      ₹{(Number(item.price?.replace(/[^0-9]/g, "")) * (item.quantity || 1)).toLocaleString()}
                    </p>
                  </div>
                </div>
              ))}
              ...
              <div className="bg-[#311B14] text-white p-8 rounded-3xl shadow-2xl border border-white/10">
                ...
                <div className="space-y-4 text-sm font-light border-b border-white/10 pb-6 mb-6">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-green-400 uppercase text-[10px] tracking-widest font-bold">Complimentary</span>
                  </div>
                  <div className="flex justify-between">
                ...
                <button className="w-full bg-[#C9A227] text-black font-bold py-4 rounded-xl text-xs uppercase tracking-[0.2em] hover:bg-white transition-all shadow-xl active:scale-95">
                  Proceed to Checkout
                </button>
